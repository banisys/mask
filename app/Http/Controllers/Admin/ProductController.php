    ...
    public function search(Request $request)
    {
        if (isset($request->cat)) {
            if ($request->cat === '999999') {
                $data = Product::with('cat')->orderBy('created_at', 'desc')->paginate(15);
            } else {
                $data = Product::where('cat_id', $request->cat)->with('cat')->orderBy('created_at', 'desc')->paginate(15);
            }
        }
        ...
        if (isset($request->brand) && isset($request->less)) {
            $data = Product::where('brand', 'like', '%' . $request->brand . '%')->with('cat')
                ->where('price', '<', $request->price)->orderBy('created_at', 'desc')->paginate(15);
            return response()->json($data);
        }
